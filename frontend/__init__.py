...
class API:
    ...
    def ping(self) -> str:
        with httpx.Client(timeout=10) as c:
            r = c.get(self._url("/health")); r.raise_for_status(); return r.text
    ...
    def process_receipt(self, uploaded_file) -> Dict[str, Any]:
        ...
        with httpx.Client(timeout=self.timeout) as c:
            r = c.post(self._url("/process_receipt"), files=files); r.raise_for_status(); return r.json()

    def send_to_sheets(self, uploaded_file) -> Dict[str, Any]:
        files = {"file": (uploaded_file.name, io.BytesIO(uploaded_file.getvalue()), uploaded_file.type)}
        with httpx.Client(timeout=self.timeout) as c:
            r = c.post(self._url("/send_to_sheets"), files=files); r.raise_for_status(); return r.json()

    def save_receipt(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        with httpx.Client(timeout=60) as c:
            r = c.post(self._url("/save_receipt"), json=payload); r.raise_for_status(); return r.json()

    def get_summary(self, period: str = "daily") -> Dict[str, Any]:
        with httpx.Client(timeout=60) as c:
            r = c.get(self._url("/summary"), params={"period": period}); r.raise_for_status(); return r.json()
    ...
    def chat(self, messages: List[Dict[str, str]], params: Optional[Dict[str, Any]] = None, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        with httpx.Client(timeout=self.timeout) as c:
            r = c.post(self._url("/chat"), json=payload); r.raise_for_status(); return r.json()
